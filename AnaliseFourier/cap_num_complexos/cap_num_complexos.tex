...
\end{resp}
\begin{exer} Deduza as seguintes identidades trigonométricas
\begin{itemize}
...
\begin{obs}[Números complexos] Sobre a notação, vale observar:~
\begin{itemize} 
...
\item[e)] Dizemos que $Z$ é nulo se tanto a parte imaginária como a real são nulas. 
\item[f] Se $Z=a+bi$, definimos o complexto conjugado de $Z$, $Z*$ ou $\bar{Z}$ de $a-bi$. É fácil ver que $Z+\bar{Z}=2a$ e $Z\bar{Z}=a^2+b^2$, isto é, números reais puros.
\end{itemize}
\end{obs}

\begin{obs}[Potências da unidade imaginária] Sobre as potências de $i$, vale observar:
    \begin{itemize}
       \item[a)] $i^3$ = $i^2i = (-1)i = -i$
       \item[b)] $i^4$ = $i^3i = (-i)i = -i^2=1$
    \end{itemize}
\end{obs}

\begin{obs}[Divisão de números complexo] A multiplicação de números complexos adimite uma operação inversa, isto é para todo complexo $Z=a+bi$ não nulo, existe um único número complexo $Z^{-1}$ tal que $ZZ^{-1}Z^{-1}Z=1$. Este número complexo é dado por:
...
\begin{exer}Escreva os seguintes números complexos na forma exponencial. Calcule também o complexo conjugado de cada um. Represente-os no plano complexo e identifique no gráfico as partes real e complexa, o argumento e o módulo.
\begin{itemize}
\item[a)] $2+3i$
\item[b)] $-2+3i$
\item[c)] $3-4i$
...
\begin{exer} Simplique as expressões numéricas.
\begin{itemize}
...
\begin{exer}\label{familiarize} Entenda e familiarize-se com as seguintes identidades e observe a primeira identidade implica todas as outras (este problema é realmente muito importante):
    \begin{itemize}
...
\begin{exer} Sejam $Z_1=Ae^{i\phi_a}$ e $Z_2=Be^{i\phi_b}$ com $A$ e $B$ positivos. Verifique que $|Z_1+Z_2|=|A+Be^{i\phi}|$ com $\phi=\phi_b-\phi_a$. Obtenha que $|Z_1+Z_2| = \sqrt{A^2+2AB\cos(\phi)+B^2}$. Conclua que $|Z_1+Z_2|=A+B$ só acontece quando $\cos(\phi)=1$. Qual a intepretação geométrica? 
\end{exer}
\begin{resp}
    A interpretação geométrica é que o módulo da soma é igual à soma dos módulos somente quando $Z_1$ e $Z_2$ têm a mesma direção e sentido no plano complexo.
\end{resp}

\begin{exer} Calcule o produto e simplifique usando as fórmulas da soma de arco para o seno e o cosseno.
...
\begin{equation}\left[\cos(\theta)+i\sen(\theta)\right]^n=\cos(n\theta)+i\sen(n\theta), ~n=0,1,2,3,\ldots
\end{equation}
\end{exer}
\begin{exer} Use a identidade anterior para calcular a razão
\begin{equation}\frac{1}{\left[\cos(\theta)+i\sen(\theta)\right]^n}=\cos(n\theta)-i\sen(n\theta), ~n=0,1,2,3,\ldots
\end{equation}
...
\begin{exer}Expresse as funções trigonométricas em termos de exponenciais e use o binômio de Newton para obter as sequintes expressões. 
\begin{eqnarray*}
